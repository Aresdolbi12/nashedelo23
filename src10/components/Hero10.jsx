...
/* Один набор городов = одна «копия» ленты. Рендерим четыре копии подряд и
   гоняем их по кругу через wrap(): пока одна уходит влево, следующая уже на месте. */
function CityRow() {
  return (
...
/* Лента, реагирующая на скорость прокрутки: в покое ползёт с базовой скоростью,
   при скролле разгоняется и на скролле вверх разворачивается — приём Framer
   Motion «ParallaxText» (useVelocity → useSpring → useAnimationFrame). */
function VelocityMarquee({ baseVelocity = -1.4 }) {
  ...
  // Четыре копии → шаг переноса 25% ширины трека
  ...
  return (
    <div className="marquee10">
      <motion.div className="marquee10-track" style={reduceMotion ? undefined : { x }}>
        <CityRow />
        <CityRow />
        <CityRow />
        <CityRow />
  ...
  return (
    ...
    >
      ...
      <div className="relative flex-1 flex flex-col justify-center">
        {/* Логотип: спокойное кинематографичное появление */}
      ...
      {/* Лента городов и дат: реагирует на скорость прокрутки */}
      <motion.div

...
/* «О нас пишут» v33: РЕАЛЬНЫЕ публикации о старте программы (21.07.2026)
   и более поздние материалы (канал администрации края в MAX, 06.08.2026).
...
const ROWS = [
  {
    outlet: 'Администрация края в MAX',
    title: 'В Краснодарском крае продолжается приём заявок на бизнес-обучение для участников СВО и их семей',
    href: 'https://max.ru/admkrai/AZ_R7U5mZD8',
    // ссылка ведёт в канал MAX и в браузере не откроется — предупреждаем,
    // чтобы клик не выглядел сломанным (сам канал так хочет заказчик)
    note: 'Откроется в приложении MAX',
  },
  {
    outlet: 'РИА Новости',
    ...
    <section id="press" className="relative px-6 lg:px-10 py-24 md:py-32">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="space-y-6 md:space-y-8 max-w-3xl">
          {ROWS.map(({ outlet, title, href, note }, i) => (
            <motion.article
              key={outlet}
            ...
            >
              <div className="plaque22 newsplaque31 relative px-8 py-7 sm:px-10 sm:py-8">
                ...
                {note && (
                  <p className="text-[#623b2a]/70 text-[12px] mt-3 leading-snug">{note}</p>
                )}
              </div>
            </motion.article>
          ))}

  ...
  return (
    <section id="about" className="relative px-6 lg:px-10 py-24 md:py-32">
      <div className="max-w-6xl mx-auto">
        {/* Тезисы v29: вехи вдоль пунктирной тропы маршрута */}
        <div className="relative pl-8 md:pl-10 max-w-3xl">
          <span className="absolute left-[9px] top-3 bottom-3 border-l-2 border-dashed border-[#d9bfa8]/35" aria-hidden="true" />
          <div className="space-y-5">
            {THESES.map(([title, text], i) => (
              <motion.div
                key={title}
                className="wp29 relative p-5 md:p-6"
                initial={{ opacity: 0, x: -26 }}
                whileInView={{ opacity: 1, x: 0 }}
                viewport={{ once: true, margin: '-50px' }}
                transition={{ duration: 0.55, ease: EASE, delay: i * 0.07 }}
              >
                <span className="wp29-d absolute -left-[29px] md:-left-[37px] top-7 w-3.5 h-3.5 rotate-45 border-2 border-[#c58b68] bg-[#0d2f22]" aria-hidden="true" />
                <div className="text-[#f2ece3] font-black text-lg md:text-xl leading-snug">{title}</div>
                <div className="text-[#d9c9b8]/85 text-sm md:text-base leading-relaxed mt-2">{text}</div>
              </motion.div>
            ))}
          </div>
        </div>

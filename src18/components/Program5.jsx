...
export default function Program5() {
  ...
  return (
    <section id="program" className="relative px-3 sm:px-4 py-24 md:py-32">
      {/* Тёмно-зелёная полоса-акцент (bold blocking) на бумажной странице */}
      <div
        className="relative max-w-[1400px] mx-auto bg-[#154734] border-2 border-[#27251f] shadow-[10px_10px_0_0_#27251f] px-5 sm:px-8 lg:px-16 py-16 md:py-24 overflow-hidden"
        style={{ backgroundImage: 'radial-gradient(ellipse at 80% -10%, rgba(217,191,168,0.14), transparent 55%)' }}
      >
        <div className="secnum16 secnum16-ondark absolute right-6 top-8 text-[min(14vw,150px)]" aria-hidden="true">02</div>
      <div className="max-w-5xl mx-auto relative">
        <span className="eyebrow16 eyebrow16-invert mb-7 inline-flex">Два дня + вебинары</span>
        <motion.h2
        ...
        >
          ...
          <div className="flex flex-wrap gap-2 mb-5" role="tablist" aria-label="Разделы программы">
            {TABS.map(({ key, label }) => (
              <button
                ...
                onClick={() => setTab(key)}
                className={`relative rounded-none px-7 py-3 font-bold text-sm md:text-base transition-colors cursor-pointer ${
              ...
              >
                {tab === key && (
                  <motion.span
                    ...
                    className="absolute inset-0 bg-[#f2e9de] rounded-none shadow-[4px_4px_0_0_#0d2f22]"
          ...
          <div className="tray16-dark"><div className="panel p-7 md:p-10 min-h-[340px]">

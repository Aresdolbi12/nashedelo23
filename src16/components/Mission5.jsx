...
function DrawnRule({ delay = 0 }) {
  return (
    <motion.div
      className="h-px bg-[rgba(39,37,31,0.16)] origin-left"
      initial={{ scaleX: 0 }}
          ...
          {STATS.map(({ value, suffix, label }, i) => (
            <div key={label}>
              ...
              >
                ...
                <div className="hidden md:block text-[#c58b68] text-xs font-bold tracking-[0.25em] uppercase">
        ...
        <div className="mt-20 md:mt-28 grid lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-8">
            {RESULTS.map((item, i) => (
              <motion.div
                key={item}
                className="border-t border-[rgba(39,37,31,0.16)] py-6 flex items-baseline gap-6 group"
                initial={{ opacity: 0, y: 18 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true, margin: '-40px' }}
                transition={{ duration: 0.6, ease: EASE, delay: i * 0.06 }}
              >
                <span className="text-[#c58b68] font-bold text-sm tracking-widest flex-shrink-0" style={{ fontFamily: "'Unbounded', sans-serif" }}>
                  {String(i + 1).padStart(2, '0')}
                </span>

      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5">
          {THESES.map(([title, text], i) => (
            <motion.div
              key={title}
              className="th28 relative p-6 md:p-7"
              initial={{ opacity: 0, y: 18 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true, margin: '-60px' }}
              transition={{ duration: 0.6, ease: EASE, delay: i * 0.07 }}
            >
                            <div className="th28-t relative text-[#f2ece3] font-black uppercase text-lg md:text-xl leading-snug">{title}</div>
              <div className="th28-d relative text-[#d9c9b8]/85 text-sm md:text-base leading-relaxed mt-2.5">{text}</div>
            </motion.div>
          ))}
        </div>
        ...
        <div className="mt-20 md:mt-24 grid lg:grid-cols-12 gap-10">
          <motion.h2
            className="lg:col-span-4 relative text-white font-black uppercase leading-[0.95] text-[clamp(2.2rem,5vw,4rem)]"
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, margin: '-60px' }}
